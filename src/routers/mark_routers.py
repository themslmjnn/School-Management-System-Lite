...
@router.post("/marks", response_model=MarkResponsePublic, status_code=status.HTTP_201_CREATED)
def put_mark(
        db: db_dependency, 
        user: user_dependency, 
        mark_request: MarkCreateTeacher):
...
@router.put("/mark/{mark_id}/update", response_model=MarkResponsePublic, status_code=status.HTTP_200_OK)
...
@router.get("/marks", response_model=list[MarkResponsePublic], status_code=status.HTTP_200_OK)
def get_groups(
        db: db_dependency, 
        user: user_dependency):
