...
@router.post(
    "/groups", response_model=GroupResponseAdmin, status_code=status.HTTP_201_CREATED
)
def add_group(
    db: db_dependency, user: user_dependency, group_request: GroupCreateAdmin
):
...
@router.put(
    ...
    response_model=GroupResponseAdmin,
...
def update_group_info(
    ...
    group_id: int,
    group_update_info_request: GroupUpdateInfoAdmin,
):
...
@router.get(
    "/groups", response_model=list[GroupResponseAdmin], status_code=status.HTTP_200_OK
)
