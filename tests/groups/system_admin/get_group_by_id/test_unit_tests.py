...
class TestGetGroupById:
    async def test_returns_correct_data(self, session: AsyncSession):
        group = await make_group(session, name="DETAIL GG", academic_year=2025)

        result = await GroupServiceAdmin.get_group_by_id(session, group.id)
    ...
    async def test_not_found_raises(self, session: AsyncSession):
        with pytest.raises(GroupNotFoundError):
            await GroupServiceAdmin.get_group_by_id(session, 999_999)
    ...
    async def test_populates_cache_after_db_hit(
        self, session: AsyncSession, mock_set_cache_group_system_admin, mocker
    ):
        group = await make_group(session, name="CACHE DET GG", academic_year=2025)

        await GroupServiceAdmin.get_group_by_id(session, group.id)

        mock_set_cache_group_system_admin.assert_called_once_with(
            GroupCacheKey.group_detail_key_admin(group.id),
            mocker.ANY,
            900,
        )

    async def test_does_not_hit_db_on_cache_hit(self, session: AsyncSession, mocker):
        group = await make_group(session, name="CACHE HIT GG", academic_year=2025)
    ...
    async def test_returns_same_data_on_cache_hit(self, session: AsyncSession):
        group = await make_group(session, name="CACHE SAME GG", academic_year=2025)

        first = await GroupServiceAdmin.get_group_by_id(session, group.id)
        second = await GroupServiceAdmin.get_group_by_id(session, group.id)
