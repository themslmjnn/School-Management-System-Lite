...
class TestGetStudentById:
    async def test_returns_correct_data(self, session: AsyncSession, student: User):
        result = await UserServiceAdmin.get_student_by_id(session, student.id)
    ...
    async def test_returns_404_when_not_found(self, session: AsyncSession):
        with pytest.raises(UserNotFoundError):
            await UserServiceAdmin.get_student_by_id(session, 999_999)
    ...
    async def test_returns_404_for_non_student_role(
        ...
        with pytest.raises(UserNotFoundError):
            await UserServiceAdmin.get_student_by_id(session, non_student.id)
    ...
    async def test_returns_same_data_on_cache_hit(
        ...
        first = await UserServiceAdmin.get_student_by_id(session, student.id)
        second = await UserServiceAdmin.get_student_by_id(session, student.id)
