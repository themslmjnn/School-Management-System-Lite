...
@pytest.fixture
def mock_set_cache_user_shared(mocker):
    return mocker.patch("src.users.services.shared.set_cache", new_callable=AsyncMock)


@pytest.fixture
def mock_set_cache_email(mocker):
    return mocker.patch("src.emails.service.set_cache", new_callable=AsyncMock)


@pytest.fixture
def mock_set_cache_subject_system_admin(mocker):
    return mocker.patch(
        "src.subjects.services.system_admin.set_cache", new_callable=AsyncMock
    )


@pytest.fixture
def mock_set_cache_subject_director(mocker):
    return mocker.patch(
        "src.subjects.services.director.set_cache", new_callable=AsyncMock
    )
...
@pytest.fixture
def mock_set_cache_group_director(mocker):
    return mocker.patch(
        "src.groups.services.director.set_cache", new_callable=AsyncMock
    )
